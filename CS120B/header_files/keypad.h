...
unsigned char GetKeypadKey() {
	...
	if ( GetBit(~KEYPADPIN,ROW1) ) { return 'D'; }
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '#'; }
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '0'; }
	if ( GetBit(~KEYPADPIN,ROW4) ) { return '*'; }

	// Check keys in col 2
	KEYPADPORT = SetBit(0xFF,COL2,0); // Set Px5 to 0; others 1
	asm("nop"); // add a delay to allow PORTx to stabilize before checking
	if ( GetBit(~KEYPADPIN,ROW1) ) { return 'C'; }
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '9'; }
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '8'; }
	if ( GetBit(~KEYPADPIN,ROW4) ) { return '7'; }

	// Check keys in col 3
	KEYPADPORT = SetBit(0xFF,COL3,0); // Set Px6 to 0; others 1
	asm("nop"); // add a delay to allow PORTx to stabilize before checking
	if ( GetBit(~KEYPADPIN,ROW1) ) { return 'B'; }
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '6'; }
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '5'; }
	if ( GetBit(~KEYPADPIN,ROW4) ) { return '4'; }

	// Check keys in col 4
	KEYPADPORT = SetBit(0xFF,COL4,0); // Set Px7 to 0; others 1
	asm("nop"); // add a delay to allow PORTx to stabilize before checking
	if (GetBit(~KEYPADPIN,ROW1) ) { return 'A'; }
	if (GetBit(~KEYPADPIN,ROW2) ) { return '3'; }
	if (GetBit(~KEYPADPIN,ROW3) ) { return '2'; }
	if (GetBit(~KEYPADPIN,ROW4) ) { return '1'; }
